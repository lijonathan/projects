...
public class Packet {
	...
	public boolean unpack(byte[] buf, int bufLen) {
		...
		for (int i = 1; i < lines.length; i++) {
			...
			/*else if (left.equals("predInfo")) {
				...
				String ip = chunks[0];
				int port_number = Integer.parseInt(chunks[1]);
				int hash = Integer.parseInt(chunks[2]);
				InetSocketAddress address = new InetSocketAddress(ip, port_number);
				senderInfo = new
					Pair<InetSocketAddress,Integer>(
							address, hash);
				...
				int begin = Integer.parseInt(chunks[0]);
				int end = Integer.parseInt(chunks[1]);
				hashRange = new
					Pair<Integer,Integer>(begin, end);
			}
				...
				int port_number = Integer.parseInt(chunks[1]);
				relayAdr = new InetSocketAddress(hostname,
						port_number);
			}
	...
	public String toString() {
		...
		if(succInfo != null){
			...
			s.append(":");
			s.append(succInfo.left.getPort());
			s.append(":");
			s.append(succInfo.right);
			s.append("\n");
		}
		if (senderInfo != null) {
			s.append("senderInfo:");
